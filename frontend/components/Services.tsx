  ...
  return (
    <section id="servicios" data-testid="services-section" className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
      <div className="mb-16">
        ...
        <div className="h-1 w-20 bg-gradient-to-r from-[#38BDF8] to-transparent"></div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {services.map((service, index) => (
          <motion.div 
            ...
            viewport={{ once: true }}
            className="bg-[#0F2640] p-8 rounded-xl border border-[#1A3652] hover:border-[#38BDF8]/30 hover:shadow-lg hover:shadow-[#38BDF8]/5 transition-all group"
            data-testid={`service-card-${index}`}
          >
            <div className="mb-6">
              <div className="w-12 h-12 bg-[#38BDF8]/10 rounded-lg flex items-center justify-center mb-4 group-hover:bg-[#38BDF8]/20 transition-colors">
                <service.icon className="w-6 h-6 text-[#38BDF8]" />
              </div>
            ...
            <ul className="space-y-3">
              {service.items.map((item, i) => (
                <li key={i} className="flex items-start gap-3">
                  <CheckCircle2 className="w-4 h-4 text-[#38BDF8]/60 shrink-0 mt-1" />
